...
$$\begin{gather*}
a \ge rb \iff a \ge \left\lceil rb \right\rceil\!, \brk[0]{\quad} \brk ab \ge n \iff a \ge \left\lceil \frac{n}{b} \right\rceil.
\end{gather*}$$
...
Let $k=\left\lfloor r(b_0+1) \right\rfloor.$ Because $r(b_0+1)>a_w,$ the integer $a_w < r(b_0+1)$ cannot exceed the greatest integer $\le r(b_0+1);$ hence $a_w \le k.$ Consequently  
...
- The algorithm finds the smallest integer $a$ satisfying $a \ge r \lceil n/a \rceil.$ Hence  
  $$\begin{gather*}
  a-1 < r \lceil n/(a-1) \rceil \quad \text{and} \brk[0]{\quad} \brk a \ge r b,\;\; b = \lceil n/a \rceil.
  \end{gather*}$$
  Because $a$ is the *smallest* such integer, the gap $a - r b$ is strictly less than 1.  
- For $r > 1,$ we have  
  $$
  {\frac{a}{r} - b = \frac{a - rb}{r}} < {\frac{1}{r} < 1} \implies {\frac{a}{r} < b+1}.
  $$
